...
for mm in range(1):
    ...
    data = Dataset(directorydata + 'LRPMap_Z-neg_Obs' + '_' + variq + '_' + savename + '.nc')
    lat = data.variables['lat'][:]
    lon = data.variables['lon'][:]                             
    lrp_SPEAR_zn = data.variables['LRP'][:].reshape(years.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    
    data = Dataset(directorydata + 'LRPMap_E-neg_Obs' + '_' + variq + '_' + savename + '.nc')
    lrp_SPEAR_en = data.variables['LRP'][:].reshape(years.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    
    data = Dataset(directorydata + 'LRPMap_IG-neg_Obs' + '_' + variq + '_' + savename + '.nc')
    ...
    ### Variable
    mask = lrp_SPEAR_z.copy()
    mask[np.where(mask!=0)] = 1.
    lrp_SPEAR_z = gaussian_filter(lrp_SPEAR_z,sigma=sigmafactor,order=0)*mask
    lrp_SPEAR_z[np.where(lrp_SPEAR_z == 0)] = np.nan
    lrp_SPEAR_z = lrp_SPEAR_z/np.nanmax(abs(lrp_SPEAR_z))
    ...
    cs1 = m.contourf(x,y,lrp_SPEAR_z,limit,extend='both',latlon=True)
    cs1.set_cmap(cmap) 
    ...
    ### Variable
    mask = lrp_SPEAR_ig.copy()
    mask[np.where(mask!=0)] = 1.
    lrp_SPEAR_ig = gaussian_filter(lrp_SPEAR_ig,sigma=sigmafactor,order=0)*mask
    lrp_SPEAR_ig[np.where(lrp_SPEAR_ig == 0)] = np.nan
    lrp_SPEAR_ig = lrp_SPEAR_ig/np.nanmax(abs(lrp_SPEAR_ig))
    ...
    cs1 = m.contourf(x,y,lrp_SPEAR_ig,limit,extend='both',latlon=True)
    cs1.set_cmap(cmap) 
