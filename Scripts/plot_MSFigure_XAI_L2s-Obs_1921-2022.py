...
for rr in range(len(ridgePenaltyall)):
    ...
    pickyears = np.where(years >= 2005)[0]
    lrp_SPEAR_zall = np.nanmean(lrp_SPEAR_z[pickyears,:,:],axis=0)
    ...
    cmap = cmr.fusion_r
    label = r'\textbf{LRP$_{z}$ Rule [Relevance] -- NClimGrid for 2005-2022}'
...
plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-%s_forL2s-Obs_%s_%s_%s.png' % (dataset,variq,monthlychoice,reg_name),dpi=600)
