...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Dark_Figures/Scores/' 
...
if maetype == 'vala':
    ...
    maetypes = '1921-2100'
modelsall = ['SPEAR_MED','SPEAR_MED_FA','FLOR']
...
###############################################################################
### Graph for accuracy
labels = [r'\textbf{SPEAR-MED}',r'\textbf{SPEAR-MED-FA}',
          r'\textbf{FLOR}']
...
ax.yaxis.grid(zorder=1,color='w',alpha=0.7,clip_on=False,linewidth=2)

def set_box_color(bp, color):
    plt.setp(bp['boxes'],color=color)
    plt.setp(bp['whiskers'], color=color,linewidth=7)
    plt.setp(bp['caps'], color='k',alpha=0)
    plt.setp(bp['medians'], color='k',linewidth=3)

positionsq = np.arange(len(labels))
bpl = plt.boxplot(plotdata,positions=positionsq,widths=0.8,
...
for i in range(plotdata.shape[1]):
    ...
    plt.plot(x, y,color='crimson', alpha=0.8,zorder=10,marker='.',linewidth=0,markersize=9,markeredgewidth=0)
 
plt.yticks(np.arange(0,101,5),list(map(str,np.round(np.arange(0,101,5),2))),
            fontsize=10) 
plt.ylim([0,15])

plt.text(-0.2,-1.4,r'\textbf{%s}' % labels[0],fontsize=11,color='w',
          ha='left',va='center')
plt.text(0.73,-1.4,r'\textbf{%s}' % labels[1],fontsize=11,color='w',
          ha='left',va='center')
plt.text(1.9,-1.4,r'\textbf{%s}' % labels[2],fontsize=11,color='w',
          ha='left',va='center')

    
plt.ylabel(r'\textbf{MAE [years]}',color='darkgrey',fontsize=12)
    
fig.suptitle(r'\textbf{%s--%s--%s [%s]}'  % (variq,months,region,maetypes),fontsize=20,color='darkgrey')

plt.tight_layout()
plt.savefig(directoryfigure + 'validationMAE-%s_BestANNModel_%s_%s_%s_CompareModels-MEDS.png' % (maetype,variq,months,region),dpi=300)
