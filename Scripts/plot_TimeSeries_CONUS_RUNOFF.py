"""
Compare regional mean RUNOFF anomalies for different SPEAR experiments
...
for i in range(len(slicemonthn)):
    ...
    ### Return data
    obs_avg, spear_avg, natural_avg = readData('RUNOFF',slicemonth,slicenan,years)
    ...
    plt.yticks(np.round(np.arange(-25,25.01,0.25),2),np.round(np.arange(-25,25.01,0.25),2))
    plt.xlim([1920,2100])
    plt.ylim([-1,1])
    
    plt.title(r'\textbf{%s: RUNOFF}' % slicemonthname,
                        color='k',fontsize=17)
    plt.ylabel(r'\textbf{Anomalies relative to 1981-2010 [mm/day]}',fontsize=11,
                          color='dimgrey')
    
    plt.tight_layout()        
    
    ### Save figure
    plt.savefig(directoryfigure+'%s_RUNOFF-CONUS_1921-2100.png' % slicemonth,dpi=300)
