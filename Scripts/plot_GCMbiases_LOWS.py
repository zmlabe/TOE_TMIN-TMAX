"""
Plot biases for SPEAR_LOW over CONUS
...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/Bias/LOW/' 
...
elif dataset == 'SPEAR_LOW_NATURAL':
    scenario = 'NATURAL'
    years = np.arange(1921,2100+1,1)
elif dataset == 'SPEAR_LOW_Scenario':
    scenario = 'SSP245'
    years = np.arange(1921,2100+1,1)

if any([dataset_obs == 'NClimGrid_LOWS']):
...
for m in range(len(monthlychoiceq)):
    ...
    if ocean_only == True:
        ...
        print('*Removed land*')
    if CONUS_only == True:
        data, data_obs = dSS.mask_CONUS(data,data_obs,'LOWS',lat_bounds,lon_bounds)
    ...
    label = r'\textbf{Bias -- %s minus %s [%s; $^{\circ}$C]}' % (dataset,dataset_obs,variq)
