"""
Compare regional mean WA anomalies for different SPEAR experiments
...
for i in range(len(slicemonthn)):
    ...
    ### Return data
    obs_avg, spear_avg = readData('WA',slicemonth,slicenan,years)
    ...
    plt.yticks(np.round(np.arange(-25,25.01,0.5),2),np.round(np.arange(-25,25.01,0.5),2))
    plt.xlim([1920,2100])
    plt.ylim([-1.5,1.5])
    
    plt.title(r'\textbf{%s: WA}' % slicemonthname,
                        color='k',fontsize=17)
    ...
    ### Save figure
    plt.savefig(directoryfigure+'%s_WA-CONUS_1921-2100.png' % slicemonth,dpi=300)
