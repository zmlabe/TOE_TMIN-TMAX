...
resolution = 'MEDS'
datasetsingleall = ['SPEAR_MED','SPEAR_MED','SPEAR_MED']
datasetnamelist = ['SPEAR_MED_SSP585','SPEAR_MED_SSP585','SPEAR_MED_SSP585']
...
variqall = ['T2M','TMAX','TMIN']
variqname = ['TAVG','TMAX','TMIN']
reg_name = 'US'
...
for lll in range(len(datasetsingleall)):
    ...
    plt.text(1950,2074,r'\textbf{%s}' % variqname[lll],fontsize=20,color='dimgrey')

### Save the predict the year plot
plt.tight_layout()
plt.savefig(directoryfigure + 'SPEAR_MED_prediction_TMINTMAX.png',dpi=600)  
