"""
Function(s) reads in monthly data from SPEAR_MED_AMIP for different variables 
...
Usage
-----
    [1] read_SPEAR_MED_AMIP(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
"""

def read_SPEAR_MED_AMIP(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    """
    Function reads monthly data from SPEAR_MED_AMIP
    ...
    Usage
    -----
    read_SPEAR_MED_AMIP(directory,vari,sliceperiod,sliceshape,
                             slicenan,numOfEns,timeper)
    """
    print('\n>>>>>>>>>> STARTING read_SPEAR_MED_AMIP function!')
    ...
    for i,ensmember in enumerate(ens):
        filename = directory + 'SPEAR/SPEAR_c192_pres_OISST_HIST_AllForc_IC1901_R50//monthly/%s/%s_%02d_1901-2020.nc' % (vari,vari,ensmember)
        ...
        print('Completed: read *SPEAR_MED_AMIP* Ensemble Member --%s--' % ensmember)
    ...
    print('Completed: read all SPEAR_MED_AMIP Members!\n')
    ...
    ###########################################################################
    ### Change units
    if any([vari=='T2M',vari=='SST',vari=='TS',vari=='TMAX',vari=='TMIN',vari=='TS']):
    ...
    ###########################################################################
    ### Select years of analysis (1901-2020)
    if timeper == 'all':
    ...
    print('>>>>>>>>>> ENDING read_SPEAR_MED_AMIP function!')    
...
# numOfEns = 30
# timeper = 'historical'
# lat,lon,var = read_SPEAR_MED_AMIP(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper)
