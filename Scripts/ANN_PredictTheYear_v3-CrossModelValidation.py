...
Version     : 3 (cross model validation approach for predictions)
"""
...
import calc_LRPclass_tf27_v1 as LRP
import sys
...
###############################################################################
###############################################################################
### Decide whether to do cross-model validation
dataset_inference = 'SPEAR_MED_LM42p2'
...
###############################################################################
        ...
        def beginFinalPlot(YpredTrain,YpredTest,Ytrain,Ytest,testIndices,years,yearsobs,YpredObs,YpredEnsembleMember):
            """
            ...
            if dataset_inference: 
                if dataset_inference == 'SPEAR_MED_NOAER':
                    yearsalli = [np.arange(1921,2020+1,1)]
                elif dataset_inference == 'SPEAR_MED_LM42p2':
                    yearsalli = [np.arange(1921,2070+1,1)]
                else:
                    yearsalli = yearsall
                plt.plot(yearsalli[sis],YpredEnsembleMember,'x',color='darkblue',
                         label=r'\textbf{%s} - 1 ensemble member' % dataset_inference,clip_on=False)
            ...
            plt.savefig(directoryfigure+savefigName+'_%s_Inference-%s.png' % (monthlychoice,dataset_inference),
                        dpi=300)  
...
###############################################################################         
        ...
        ###############################################################
        ###############################################################
        ### Try another ensemble member for cross-model validation
        if dataset_inference:
            if dataset_inference == 'SPEAR_MED_Scenario':
                scenarioi = 'SSP245'
            else:
                scenarioi = scenario
            testingEnsembleMemberS = dSS.read_InferenceLargeEnsemble(variq,dataset_inference,dataset_obs,monthlychoice,
                                                                    scenarioi,resolution,lat_bounds,
                                                                    lon_bounds,rm_annual_mean,rm_merid_mean,
                                                                    land_only,ocean_only,CONUS_only,
                                                                    baselineSTD,yrminb,yrmaxb)
        else:
            YpredEnsembleMember = np.nan
        ###############################################################
        ###############################################################
        ###############################################################
        ...
        YpredObs = convert_fuzzyDecade_toYear(model.predict(XobsS),
                                              ...
                                              yearsall[sis])
        if dataset_inference:
            YpredEnsembleMember = convert_fuzzyDecade_toYear(model.predict(testingEnsembleMemberS),
                                                             startYear,
                                                             classChunk,
                                                             yearsall[sis])
        ...
        ### Create final plot
        beginFinalPlot(YpredTrain,YpredTest,Ytrain,Ytest,testIndices,years,yearsobs,YpredObs,YpredEnsembleMember)
