"""
Calculate trend for different SPEAR_MED periods for SNOW
...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/Trend/SNOW/' 
...
for s in range(len(timeperiodq)):
    ...
    for m in range(len(monthlychoiceq)):
        ...
        if timeperiod == 'satellite':
            ...
            label = r'\textbf{%s Trend [m/decade] for %s-%s in %s}' % (variq,1979,2020,monthlychoice)
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [m/decade] for %s-%s in %s}' % (variq,1997,2014,monthlychoice)
            limit = np.arange(-0.02,0.021,0.001)
            barlim = np.round(np.arange(-0.02,0.03,0.01),2)
            cmap = cmr.waterlily
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [m/decade] for %s-%s in %s}' % (variq,1921,1978,monthlychoice)
            limit = np.arange(-0.02,0.021,0.001)
            barlim = np.round(np.arange(-0.02,0.03,0.01),2)
            cmap = cmr.waterlily
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [m/decade] in %s}' % (variq,monthlychoice)
            limit = np.arange(-0.02,0.021,0.001)
            barlim = np.round(np.arange(-0.02,0.03,0.01),2)
            cmap = cmr.waterlily
