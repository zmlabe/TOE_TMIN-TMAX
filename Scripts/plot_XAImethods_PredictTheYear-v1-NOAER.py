"""
LRP for SPEAR_NOAER epochs to see if the regions change
...
variq = 'TMIN'
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/LRP/NOAER/'
land_only = False
ocean_only = False
...
iterations = [500]
NNType = 'ANN'
testingn = 1 # ensemble members used for testing

for mm in range(len(monthlychoiceq)):
    monthlychoice = monthlychoiceq[mm]
    ###############################################################################
    ### Read in LRP after training on SPEAR_MED_NOAER
    modelType = 'TrainedOnSPEAR_MED_NOAER'
    ...
    label = r'\textbf{%s-%s -- LRP-SPEAR-NOAER [Relevance]}' % (monthlychoice,variq)
    ...
    ax1.annotate(r'\textbf{1921-1971}',xy=(0,0),xytext=(-0.04,0.5),
                  textcoords='axes fraction',color='k',fontsize=9,
    ...
    ### Variable
    lrp_SPEAR_e1 = lrp_SPEAR_e1/np.max(lrp_SPEAR_e1)
    ...
    cs1 = m.contourf(x,y,lrp_SPEAR_e1,limit,extend='max',latlon=True)
    cs1.set_cmap(cmap) 
    ...
    ax2.annotate(r'\textbf{[%s]}' % letters[3],xy=(0,0),xytext=(0.98,1.05),
                  textcoords='axes fraction',color='k',fontsize=9,
                  rotation=0,ha='center',va='center')
    ax2.annotate(r'\textbf{1971-2020}',xy=(0,0),xytext=(-0.04,0.5),
                  textcoords='axes fraction',color='k',fontsize=9,
                  rotation=90,ha='center',va='center')
    ...
    ### Variable
    lrp_SPEAR_e2 = lrp_SPEAR_e2/np.max(lrp_SPEAR_e2)
    lrp_SPEAR_e2[np.where(lrp_SPEAR_e2 == 0)] = np.nan
    ...
    cs1 = m.contourf(x,y,lrp_SPEAR_e2,limit,extend='max',latlon=True)
    cs1.set_cmap(cmap) 
    ...
    plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-SPEAR_NOAER_%s_%s.png' % (variq,monthlychoice),dpi=300)
