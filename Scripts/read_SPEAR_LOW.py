...
def read_SPEAR_LOW(directory,vari,sliceperiod,sliceshape,slicenan,numOfEns,timeper):
    ...
    ###########################################################################
    ### Parameters
    time = np.arange(1851,2100+1,1)
    mon = 12
    ens = np.arange(1,numOfEns+1,1)
    
    timesat1 = np.arange(1851,2010+1,1)
    ...
    elif any([vari=='PRECL',vari=='PRECC',vari=='PRECT',vari=='WA']):
    ...
    ###########################################################################
    ### Select years of analysis (1851-2100)
    if timeper == 'all':
        ...
        if sliceperiod == 'DJF':
            yearhistq = np.where((time >= 1921) & (time <= 2099))[0]
        else:
