"""
Calculate climatologies for different spear runs
...
modelsnamen = ['SPEAR_MED','SPEAR_MED_FA','SPEAR_MED_AMIP','SPEAR_HIGH']
scenario = 'SSP585'
...
def readModel(variq,monthlychoice,scenario,lat_bounds,lon_bounds):
    ...
    yearsq = np.where((yearsobs >= 1940) & (yearsobs <= 2020))[0]
    obs = data_obs_all[yearsq,:,:]
        
    ### Model years
    yearsspear = np.arange(1921,2100+1,1)
    yearspearq = np.where((yearsspear >= 1940) & (yearsspear <= 2020))[0]
    yearsnoaer = np.arange(1921,2020+1,1)
    yearnoaerq = np.where((yearsnoaer >= 1940) & (yearsnoaer <= 2020))[0]
    
    data_spearn = np.nanmean(data_spear[:,yearspearq,:,:] - obs,axis=(0,1))
    data_spearfan = np.nanmean(data_spearfa[:,yearspearq,:,:] - obs,axis=(0,1))
    ...
    yearsobshigh = np.arange(1940,2022+1,1)
    yearshighq = np.where((yearsobshigh >= 1940) & (yearsobshigh <= 2020))[0]
    ...
    yearspearqhigh = np.where((yearsspearhigh >= 1940) & (yearsspearhigh <= 2020))[0]
    
    data_highn = np.asarray(np.nanmean(data_spearhigh[:,yearspearqhigh,:,:] - obshighn,axis=(0,1)))
    
    return data_spearn,data_spearfan,data_highn,data_amipn,lats,lons,latshigh,lonshigh 

data_spearn_1,data_spearfan_1,data_highn_1,data_amipn_1,lats,lons,latshigh,lonshigh = readModel(variq,monthlychoiceq[0],scenario,lat_bounds,lon_bounds)
data_spearn_2,data_spearfan_2,data_highn_2,data_amipn_2,lats,lons,latshigh,lonshigh = readModel(variq,monthlychoiceq[1],scenario,lat_bounds,lon_bounds)
data_spearn_3,data_spearfan_3,data_highn_3,data_amipn_3,lats,lons,latshigh,lonshigh = readModel(variq,monthlychoiceq[2],scenario,lat_bounds,lon_bounds)
data_spearn_4,data_spearfan_4,data_highn_4,data_amipn_4,lats,lons,latshigh,lonshigh = readModel(variq,monthlychoiceq[3],scenario,lat_bounds,lon_bounds)

dataplot = [data_spearn_1,data_spearfan_1,data_amipn_1,data_highn_1,
            data_spearn_2,data_spearfan_2,data_amipn_2,data_highn_2,
            data_spearn_3,data_spearfan_3,data_amipn_3,data_highn_3,
            data_spearn_4,data_spearfan_4,data_amipn_4,data_highn_4]
...
plt.savefig(directoryfigure + 'Bias_AllGFDLModels_USA_%s-%s_Seasons.png' % (variq,dataset_obs),dpi=300)
...
plt.savefig(directoryfigure + 'Bias_AllGFDLModels_Global_%s-%s_Seasons.png' % (variq,dataset_obs),dpi=300)
