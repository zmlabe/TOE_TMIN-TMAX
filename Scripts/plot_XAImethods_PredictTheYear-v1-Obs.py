...
for mm in range(len(monthlychoiceq)):
    ...
    lrp_SPEAR_z = data.variables['LRP'][:].reshape(years.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    
    data = Dataset(directorydata + 'LRPMap_E_Obs' + '_' + variq + '_' + savename + '.nc')
    lrp_SPEAR_e = data.variables['LRP'][:].reshape(years.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    
    data = Dataset(directorydata + 'LRPMap_IG_Obs' + '_' + variq + '_' + savename + '.nc')
    lrp_SPEAR_ig = data.variables['LRP'][:].reshape(years.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    ...
    ax2.annotate(r'\textbf{[%s]}' % letters[3],xy=(0,0),xytext=(0.98,1.05),
                  textcoords='axes fraction',color='k',fontsize=9,
                  rotation=0,ha='center',va='center')
    ax2.annotate(r'\textbf{1971-2021}',xy=(0,0),xytext=(-0.04,0.5),
                  textcoords='axes fraction',color='k',fontsize=9,
                  rotation=90,ha='center',va='center')
    ...
    plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-NClimGrid_%s_%s.png' % (variq,monthlychoice),dpi=300)
