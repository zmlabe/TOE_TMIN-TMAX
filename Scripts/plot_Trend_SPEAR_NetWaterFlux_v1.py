"""
Calculate trend for different SPEAR_MED periods for P-E
...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/Trend/WFLUX/' 
...
for s in range(len(timeperiodq)):
    ...
    for m in range(len(monthlychoiceq)):
        ...
        if timeperiod == 'satellite':
            ...
            ### Get data
            lat_bounds,lon_bounds = UT.regions(reg_name)
            spear_evap,lats,lons = read_primary_dataset('EVAP','SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
            noaern_evap,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_NOAER',monthlychoice,scenario,lat_bounds,lon_bounds)
            spear_prect,lats,lons = read_primary_dataset('PRECT','SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
            noaern_prect,lats,lons = read_primary_dataset('PRECT','SPEAR_MED_NOAER',monthlychoice,scenario,lat_bounds,lon_bounds)
            lon2,lat2 = np.meshgrid(lons,lats)
            
            ### Calculate net water flux (P-E, water availability)
            spearn = spear_prect - spear_evap
            noaern = noaern_prect - noaern_evap
            ...
            label = r'\textbf{%s Trend [mm/decade] for %s-%s in %s}' % (variq,1979,2020,monthlychoice)
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [mm/decade] for %s-%s in %s}' % (variq,1997,2014,monthlychoice)
            limit = np.arange(-0.4,0.41,0.01)
            barlim = np.round(np.arange(-0.4,0.41,0.2),2)
            cmap = cmr.waterlily
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [mm/decade] for %s-%s in %s}' % (variq,1921,1978,monthlychoice)
            limit = np.arange(-0.4,0.41,0.01)
            barlim = np.round(np.arange(-0.4,0.41,0.2),2)
            cmap = cmr.waterlily
        ...
        ###############################################################################
            ...
            ### Get data
            lat_bounds,lon_bounds = UT.regions(reg_name)
            spearn_evap,lats,lons = read_primary_dataset('EVAP','SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
            ssp245n_evap,lats,lons = read_primary_dataset('EVAP','SPEAR_MED_Scenario',monthlychoice,'SSP245',lat_bounds,lon_bounds)
            spearn_prect,lats,lons = read_primary_dataset('PRECT','SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
            ssp245n_prect,lats,lons = read_primary_dataset('PRECT','SPEAR_MED_Scenario',monthlychoice,'SSP245',lat_bounds,lon_bounds)
            lon2,lat2 = np.meshgrid(lons,lats)
            
            ### Calculate net water flux (P-E, water availability)
            spearn = spearn_prect - spearn_evap
            ssp245n = ssp245n_prect - ssp245n_evap
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [mm/decade] in %s}' % (variq,monthlychoice)
            limit = np.arange(-0.4,0.41,0.01)
            barlim = np.round(np.arange(-0.4,0.41,0.2),2)
            cmap = cmr.waterlily
