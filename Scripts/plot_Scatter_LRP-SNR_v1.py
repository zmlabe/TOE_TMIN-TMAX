...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/LRP/Scatter/SNR/'
...
for m in range(len(monthlychoiceq)):
    ...
    yearsnrlrpq = years.max() + 1 # index
    timem = np.where(years_med == yearsnrlrpq)[0][0]
    ...
    snr_scatter = SNR_spear.ravel()
    
    sigq = 0.001
    significance = np.where(lrp_scatter <= sigq)[0]
    lrp_scatter[significance] = np.nan
    
    mask = ~np.isnan(snr_scatter) & ~np.isnan(lrp_scatter)
    slope, intercept, r_value, p_value, std_err = sts.linregress(snr_scatter[mask],lrp_scatter[mask])
    ...
    plt.scatter(snr_scatter,lrp_scatter,marker='o',s=30,color='maroon',
                alpha=0.4,edgecolors='maroon',linewidth=0,clip_on=False)
    
    plt.xticks(np.arange(0,31,5),map(str,np.arange(0,31,5)),fontsize=10)
    ...
    plt.xlabel(r'\textbf{SNR for SPEAR-MED}',fontsize=11,color='dimgrey')
    plt.ylabel(r'\textbf{ANN Relevance for SPEAR-MED}',fontsize=11,color='dimgrey')
    
    plt.tight_layout()
    plt.savefig(directoryfigure + savename + '_SCATTER_LRP-SNR_v1.png',dpi=300)
