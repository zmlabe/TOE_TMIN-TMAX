...
for m in range(len(monthlychoiceq)):
    ...
    spear_ens = spear[testIndices,:,:]
    
    ### Calculate standardized anomalies
    yearclimoq = np.where((years >= 1981) & (years <= 2010))[0]
    climo = np.nanmean(np.nanmean(spear_ens[:,yearclimoq,:,:],axis=1),axis=0)
    climostd = np.nanmean(np.nanstd(spear_ens[:,yearclimoq,:,:],axis=1),axis=0)
    
    spear_ensz = (spear_ens - climo)/climostd
    ...
    lrp_SPEAR_z = data.variables['LRP'][:].reshape(testingn,years_med.shape[0],lat.shape[0],lon.shape[0])
    data.close()
    
    ### Years with relevance have 0 outside CONUS
    lrp_SPEAR_z[np.where(lrp_SPEAR_z == 0.)] = np.nan
    
    ### Take means across all years
    yeartrendlrpq = years.max() + 1 # index
    timem = np.where(years_med == yeartrendlrpq)[0][0]
    lrp_spear = lrp_SPEAR_z[:,:timem,:,:]
    ...
    ###########################################################################
    ### LRP scatter
    lrp_scatter = lrp_spear.ravel()
    trend_scatter = spear_ensz.ravel()
    ...
    plt.xticks(np.arange(-100,101,20),map(str,np.round(np.arange(-100,101,20),2)),fontsize=10)
    plt.yticks(np.arange(-1.5,1.6,0.25),map(str,np.round(np.arange(-1.5,1.6,0.25),2)),fontsize=10)
    plt.xlim([-50,50])
    plt.ylim([-1,1])
    
    plt.title('XAI for %s over %s using %s' % (monthlychoice,reg_name,variq),color='k',fontsize=15)
    
    plt.xlabel(r'\textbf{AlbedoZ for SPEAR-MED}',fontsize=11,color='dimgrey')
    plt.ylabel(r'\textbf{ANN Relevance for SPEAR-MED}',fontsize=11,color='dimgrey')
    
    plt.tight_layout()
    plt.savefig(directoryfigure + savename + '_SCATTER_LRP-AlbedoZ_v1.png',dpi=300)
