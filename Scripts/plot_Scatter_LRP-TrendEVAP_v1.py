...
for m in range(len(monthlychoiceq)):
    ...
    plt.yticks(np.arange(-0.5,0.6,0.1),map(str,np.round(np.arange(-0.5,0.6,0.1),2)),fontsize=10)
    plt.xticks(np.arange(-1,1.01,0.1),map(str,np.round(np.arange(-1,1.01,0.1),2)),fontsize=10)
    plt.ylim([-0.3,0.3])
    plt.xlim([-0.3,0.3])
    
    plt.title('XAI for %s over %s using %s' % (monthlychoice,reg_name,variscatter),color='k',fontsize=15)
    
    plt.xlabel(r'\textbf{Evaporation Trend for SPEAR-MED}',fontsize=11,color='dimgrey')
    plt.ylabel(r'\textbf{ANN Relevance for SPEAR-MED}',fontsize=11,color='dimgrey')
    
    plt.tight_layout()
    plt.savefig(directoryfigure + savename + '_SCATTER_LRP-EvaporationTrend_v1.png',dpi=300)
