"""
Calculate trend for different SPEAR_MED/SPEAR_MED_FA periods
...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/Trend/FA/' 
...
for s in range(len(timeperiodq)):
    ...
    for m in range(len(monthlychoiceq)):
        ...
        if timeperiod == 'satellite':
            ...
            noaern,lats,lons = read_primary_dataset(variq,'SPEAR_MED_FA',monthlychoice,scenario,lat_bounds,lon_bounds)
            ...
            modelname = ['SPEAR_MED','SPEAR_MED_FA']
            ...
            label = r'\textbf{%s Trend [$^{\circ}$C/decade] for %s-%s in %s}' % (variq,1979,2020,monthlychoice)
            ...
            plt.savefig(directoryfigure + 'TrendLinear_%s_%s_%s_FA.png' % (variq,monthlychoice,timeperiod),dpi=300)
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [$^{\circ}$C/decade] for %s-%s in %s}' % (variq,1997,2014,monthlychoice)
            limit = np.arange(-1,1.01,0.05)
            barlim = np.round(np.arange(-1,1.1,0.5),2)
            cmap = cmocean.cm.balance
        ...
        ###############################################################################
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [$^{\circ}$C/decade] for %s-%s in %s}' % (variq,1921,1978,monthlychoice)
            limit = np.arange(-1,1.01,0.05)
            barlim = np.round(np.arange(-1,1.1,0.5),2)
            cmap = cmocean.cm.balance
        ...
        ###############################################################################
            ...
            lat_bounds,lon_bounds = UT.regions(reg_name)
            spearn,lats,lons = read_primary_dataset(variq,'SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
            ssp245n,lats,lons = read_primary_dataset(variq,'SPEAR_MED_FA',monthlychoice,scenario,lat_bounds,lon_bounds)
            lon2,lat2 = np.meshgrid(lons,lats)
            ...
            modelname = ['SPEAR_MED','SPEAR_MED_FA','SPEAR_MED','SPEAR_MED_FA']
            ...
            ### Set limits for contours and colorbars
            label = r'\textbf{%s Trend [$^{\circ}$C/decade] in %s}' % (variq,monthlychoice)
            limit = np.arange(-1,1.01,0.05)
            barlim = np.round(np.arange(-1,1.1,0.5),2)
            cmap = cmocean.cm.balance
