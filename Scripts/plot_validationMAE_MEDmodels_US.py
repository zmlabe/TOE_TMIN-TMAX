...
for i in range(plotdata.shape[1]):
    ...
    plt.plot(x, y,color='teal', alpha=0.8,zorder=10,marker='.',linewidth=0,markersize=9,markeredgewidth=0)
 
plt.yticks(np.arange(0,101,5),list(map(str,np.round(np.arange(0,101,5),2))),
            fontsize=10) 
plt.ylim([0,15])

plt.text(-0.2,-1.4,r'\textbf{%s}' % labels[0],fontsize=11,color='dimgrey',
          ha='left',va='center')
plt.text(0.73,-1.4,r'\textbf{%s}' % labels[1],fontsize=11,color='dimgrey',
          ha='left',va='center')
plt.text(1.9,-1.4,r'\textbf{%s}' % labels[2],fontsize=11,color='dimgrey',
          ha='left',va='center')

    
plt.text(-1,31.5,r'\textbf{[%s]}' % letters[0],color='k',fontsize=6)
plt.ylabel(r'\textbf{MAE [years]}',color='k',fontsize=12)
    
fig.suptitle(r'\textbf{%s--%s--%s [%s]}'  % (variq,months,region,maetypes),fontsize=20,color='dimgrey')
