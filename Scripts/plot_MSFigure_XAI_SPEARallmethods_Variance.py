"""
Calculate LRP variance for SPEAR_MED across all years
...
for mm in range(len(monthlychoiceq)):
    ...
    ### Calculate variance
    lrp_SPEAR_z = np.nanvar(lrp_SPEAR_zq,axis=1)
    lrp_SPEAR_e = np.nanvar(lrp_SPEAR_eq,axis=1)
    lrp_SPEAR_ig = np.nanvar(lrp_SPEAR_igq,axis=1)
    ...
    limit = np.arange(0,0.201,0.001)
    barlim = np.round(np.arange(0,0.21,0.1),2)
    cmap = cmr.sunburst_r
    label = r'\textbf{Variance [Relevance]}'
    ...
    cs2 = m.contourf(x,y,lrp_SPEAR_e1,limit,extend='max',latlon=True)
    cs2.set_cmap(cmap) 
    ...
    plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-%s_AllMethods_%s_%s_%s_Variance.png' % (dataset,variq,monthlychoice,reg_name),dpi=300)
