...
for mm in range(len(maetypen)):
    ...
    plt.text(-0.15,-1.4,r'\textbf{%s}' % labels[0],fontsize=11,color='dimgrey',
              ha='left',va='center')
    plt.text(0.94,-1.4,r'\textbf{%s}' % labels[1],fontsize=11,color='dimgrey',
              ha='left',va='center')
    # plt.text(1.9,-1.4,r'\textbf{%s}' % labels[2],fontsize=11,color='dimgrey',
    #           ha='left',va='center')
    
    plt.text(1.45,13.6,r'\textbf{[%s]}' % letters[mm],fontsize=10,color='k')
    plt.text(0.5,13,r'\textbf{%s}' % maetypes,fontsize=17,color='k',ha='center')
    
        
    plt.ylabel(r'\textbf{MAE [years]}',color='dimgrey',fontsize=12)

plt.tight_layout()
plt.savefig(directoryfigure + 'validationMAE-%s_BestANNModel_%s_%s_%s_CompareModels-MEDS_temp.png' % (maetype,variq,months,region),dpi=600)
