"""
Calculate TOE for different SPEAR simulations
...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Figures/TOE/' 
...
for m in range(len(monthlychoiceq)):
    ...
    nonoaern,lats,lons = read_primary_dataset(variq,'SPEAR_MED_NOAER',monthlychoice,scenario,lat_bounds,lon_bounds)
    lon2,lat2 = np.meshgrid(lons,lats)
    
    ### Select same years
    yearq_med = np.where((years_med >= years.min()) & (years_med <= years.max()))[0]
    yearq_nonoaer = np.where((years_nonoaer >= years.min()) & (years_nonoaer <= years.max()))[0]
    
    spear = spearn[:,yearq_med,:,:]
    noaer = nonoaern[:,yearq_nonoaer,:,:]
    
    ### Calculate ensemble means
    mean_spear = np.nanmean(spear,axis=0)
    mean_nonoaer = np.nanmean(noaer,axis=0)
    ...
    window = 10 
    min_periods = 10
    smooth_noaer = np.empty((noaer.shape[0],noaer.shape[1],noaer.shape[2],noaer.shape[3]))
    for ens in range(noaer.shape[0]):
        for i in range(noaer.shape[2]):
            for j in range(noaer.shape[3]):
                smooth_noaer[ens,:,i,j] = rollingMean(noaer[ens,:,i,j],window,min_periods)
        print('Completed: SPEAR-MED-NOAER Ensemble #%s running mean!' % (ens+1))
    ...
    noaerbase = smooth_noaer[:,yearq,:,:]
    
    ### 2 Sigma of 1920-1949
    spear2 = np.nanstd(spearbase[:,:,:,:],axis=1) * 2.
    noaer2 = np.nanstd(noaerbase[:,:,:,:],axis=1) * 2.
    
    ### Limit of baseline
    spearbasemean = np.nanmean(spearbase[:,:,:,:],axis=1)
    noaerbasemean = np.nanmean(noaerbase[:,:,:,:],axis=1)

    spearlimit = spearbasemean + spear2
    noaerlimit = noaerbasemean + noaer2
    
    ### Calculate ToE
    toe_spear = calcToE(smooth_spear,spearlimit,years)
    toe_noaer = calcToE(smooth_noaer,noaerlimit,years)
    
    ### Calculate ensemble mean ToE
    mtoe_spear = np.nanmedian(toe_spear,axis=0)
    mtoe_noaer = np.nanmedian(toe_noaer,axis=0)

    ### Prepare for plotting
    runs = [mtoe_spear,mtoe_noaer]
    modelname = [r'SPEAR_MED',r'SPEAR_MED_NOAER']
    ...
    cbar.set_label(r'\textbf{TIMING OF EMERGENCE [Years] -- %s}' % monthlychoice,fontsize=11,color='dimgrey',labelpad=1.4)  
