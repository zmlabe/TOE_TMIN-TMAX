"""
Train ANN to predict the year for ToE in the CONUS for single forcing tests
...
###############################################################################
###############################################################################
### Single forcing tests
singleForcingTest = True
if singleForcingTest == False:
    singleForcingExp  = 'NOPE'
else:
    singleForcingExp  = 'anthro'
    if singleForcingExp == 'aer':
        yearsall = [np.arange(1921,2020+1,1)]
    elif singleForcingExp == 'ghg':
        yearsall = [np.arange(1921,2020+1,1)]
            ...
            for i in range(0,train_output_rs.shape[0]):
                if i == train_output_rs.shape[0]-1:
                    plt.plot(xs_test,train_output_rs[i,:],'o',
                                ...
                                label=r'\textbf{%s - Training Data [%s]}' % (singlesimulation,singleForcingExp))
                else:
                    plt.plot(xs_test,train_output_rs[i,:],'o',
                                markersize=4,color='lightgray',clip_on=False,
                                alpha=0.4,markeredgecolor='k',markeredgewidth=0.4)
            for i in range(0,test_output_rs.shape[0]):
                if i == test_output_rs.shape[0]-1:
                    plt.plot(xs_test,test_output_rs[i,:],'o',
                            ...
                            label=r'\textbf{%s - Testing Data [%s]}' % (singlesimulation,singleForcingExp))
                else:
                    plt.plot(xs_test,test_output_rs[i,:],'o',
                            markersize=4,color='crimson',clip_on=False,alpha=0.3,
                            markeredgecolor='crimson',markeredgewidth=0.4)
...
###############################################################################         
        ...
        elif singleForcingTest == True:
            if singleForcingExp  =='anthro':
                ### Read in data
                lat_bounds,lon_bounds = UT.regions(reg_name)
                data_all1,lats,lons = read_primary_dataset(variq,'SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
                data_all2,lats,lons = read_primary_dataset(variq,'SPEAR_MED_NATURAL',monthlychoice,scenario,lat_bounds,lon_bounds)
                
                ### Calculate single forcing
                meanforcing = np.nanmean(data_all2,axis=0)
                data_all = data_all1 - meanforcing
                
                ### Read in observations
                data_obs_all,lats_obs,lons_obs = read_primary_dataset(variq,dataset_obs,monthlychoice,scenario,lat_bounds,lon_bounds)
                data, data_obs, = data_all, data_obs_all,
            elif singleForcingExp  == 'aer':
                ### Read in data
                lat_bounds,lon_bounds = UT.regions(reg_name)
                data_all1,lats,lons = read_primary_dataset(variq,'SPEAR_MED',monthlychoice,scenario,lat_bounds,lon_bounds)
                data_all2,lats,lons = read_primary_dataset(variq,'SPEAR_MED_NOAER',monthlychoice,scenario,lat_bounds,lon_bounds)
                
                ### Calculate single forcing
                meanforcing = np.nanmean(data_all2,axis=0)
                data_all = data_all1[:,:len(meanforcing),:,:] - meanforcing
                ...
                ### Read in data
                lat_bounds,lon_bounds = UT.regions(reg_name)
                data_all1,lats,lons = read_primary_dataset(variq,'SPEAR_MED_NOAER',monthlychoice,scenario,lat_bounds,lon_bounds)
                data_all2,lats,lons = read_primary_dataset(variq,'SPEAR_MED_NATURAL',monthlychoice,scenario,lat_bounds,lon_bounds)
                
                ### Calculate single forcing
                meanforcing = np.nanmean(data_all2[:,:data_all1.shape[1],:,:],axis=0)
                data_all = data_all1 - meanforcing
                ...
                print(ValueError('WRONG SINGLE FORCING SETTINGS FOR TYPE!'))
                sys.exit()                
        else:
            print(ValueError('WRONG SINGLE FORCING SETTING!'))
            sys.exit()
...
###############################################################################         
        ################################################################################################################################################                
        ### Save models
        savename = modelType+'_Vari-'+variq+'_Obs-' + dataset_obs + '_Region-' + reg_name + '_Months-' + monthlychoice + '_L2-'+ str(ridge_penalty[0])+ '_LR-' + str(lr_here)+ '_Batch'+ str(batch_size)+ '_Iters' + str(iterations[0]) + '_' + NNType + str(hiddensList[0][0]) + 'x' + str(hiddensList[0][-1]) + '_SegSeed' + str(random_segment_seed) + '_NetSeed'+ str(random_network_seed) + '_SingleType-' + singleForcingExp
