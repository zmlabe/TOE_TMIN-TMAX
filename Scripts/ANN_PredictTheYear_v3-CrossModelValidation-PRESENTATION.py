...
dataset_inference = 'SPEAR_MED_AMIP'
...
for ms in range(len(monthlychoiceall)):
    ...
    for sis,singlesimulation in enumerate(datasetsingle):
        ...
        if datasetsingle[sis] == 'SPEAR_MED':
            ...
            scenario = 'SSP585'
        elif datasetsingle[sis] == 'SPEAR_MED_AMIP':
            scenario = 'SSP585'
...
###############################################################################
        ...
        def beginFinalPlot(YpredTrain,YpredTest,Ytrain,Ytest,testIndices,years,yearsobs,YpredObs,YpredEnsembleMember):
            ...
            if dataset_inference: 
                if any([dataset_inference == 'SPEAR_MED_NOAER',dataset_inference == 'SPEAR_MED_AMIP']):
                    yearsalli = [np.arange(1921,2020+1,1)]
                else:
                    yearsalli = yearsall
                plt.plot(yearsalli[sis],YpredEnsembleMember,'x',color='aqua',
                         label=r'\textbf{%s} [1 Ens.]' % dataset_inference,clip_on=False)
