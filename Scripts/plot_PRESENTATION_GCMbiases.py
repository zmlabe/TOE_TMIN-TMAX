...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Dark_Figures/Biases/' 
...
if any([dataset_obs == 'NClimGrid_MEDS']):
    ...
    timeexperi = ['1921-1971','1972-2022']
...
for m in range(len(monthlychoiceq)):
    ...
    ### Select years
    if any([dataset_obs == 'NClimGrid_MEDS']):
        yearsq = np.where((years >= 1921) & (years <= 2022))[0]
        model = data[:,yearsq,:,:]
    ...
    label = r'\textbf{Bias = SPEAR_MED minus NClimGrid [%s; $^{\circ}$C]}' % (variq)
    ...
    for i in range(len(plotdata)):
        ...
        m = Basemap(llcrnrlon=-119,llcrnrlat=22,urcrnrlon=-64,urcrnrlat=49,
                    ...
                    area_thresh=10000)
        m.drawcoastlines(color='gold',linewidth=0.5)
        m.drawstates(color='gold',linewidth=0.5)
        m.drawcountries(color='gold',linewidth=0.5)
        ...
        plt.title(r'\textbf{%s for %s}' % (timeexperi[i],slicemonthnamen[i]),fontsize=17,color='w')
        
    cbar_ax1 = fig.add_axes([0.305,0.14,0.4,0.03])                
    cbar1 = fig.colorbar(cs1,cax=cbar_ax1,orientation='horizontal',
                        extend='max',extendfrac=0.07,drawedges=False)
    cbar1.set_label(label,fontsize=8,color='w',labelpad=5)  
