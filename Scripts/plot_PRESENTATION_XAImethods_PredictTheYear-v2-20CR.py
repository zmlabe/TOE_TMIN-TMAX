...
directoryfigure = '/home/Zachary.Labe/Research/TOE_TMIN-TMAX/Dark_Figures/LRP/' 
...
for mm in range(len(monthlychoiceq)):
    ...
    timem1 = np.where((years >= 1921) & (years <= 1970))[0]
    timem2 = np.where((years > 1970) & (years <= 1990))[0]
    ...
    plt.title(r'\textbf{LRP$_{z}$ Rule}',fontsize=20,color='darkgrey')
    ax1.annotate(r'\textbf{1921-1955}',xy=(0,0),xytext=(-0.04,0.5),
    ...
    plt.title(r'\textbf{Integrated Gradients}',fontsize=20,color='darkgrey')
    
    ###############################################################################
    cbar_ax1 = fig.add_axes([0.41,0.1,0.2,0.025])                
    cbar1 = fig.colorbar(cs1,cax=cbar_ax1,orientation='horizontal',
                        extend='both',extendfrac=0.07,drawedges=False)
    cbar1.set_label(label,fontsize=10,color='w',labelpad=1.8)  
    ...
    plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-%s_for20CR_%s_%s_%s.png' % (dataset,variq,monthlychoice,reg_name),dpi=300)
