...
for mm in range(len(monthlychoiceq)):
    ...
    savename = modelType+'_Vari-'+variq+'_Obs-' + dataset_obs + '_Region-' + reg_name + '_Months-' + monthlychoice + '_L2-'+ str(ridge_penalty[0])+ '_LR-' + str(lr_here)+ '_Batch'+ str(batch_size)+ '_Iters' + str(iterations[0]) + '_' + NNType + str(hiddensList[0][0]) + '_SegSeed' + str(random_segment_seed) + '_NetSeed'+ str(random_network_seed)
    ...
    cmap = cmr.fusion_r
    label = r'\textbf{Relevance [1-layer$_{10}$ ANN]}'
    ...
    ### Variable
    mask = lrp_SPEAR_e1.copy()
    mask[np.where(mask!=0)] = 1.
    lrp_SPEAR_e1 = gaussian_filter(lrp_SPEAR_e1,sigma=sigmafactor,order=0)*mask
    ...
    plt.savefig(directoryfigure + 'PredictTheYear_LRPcomparison-%s_AllMethods_%s_%s_%s_SimplerNN.png' % (dataset,variq,monthlychoice,reg_name),dpi=300)
